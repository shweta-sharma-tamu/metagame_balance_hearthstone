...
def objective_function(card_selection, env):
    # Convert flat card_selection to a list of Minion objects
    minion_player_list = [Minion(None, *card_selection[i+1:i+4], None) for i in range(0, 30 * 3, 3)]
    minion_opponent_list = [Minion(None, *card_selection[i+1:i+4], None) for i in range(30 * 3, 60 * 3, 3)]
...
def combined_objective_function(card_selection):
    ...
    objective_func_value = objective_function(card_selection, env)
...
if __name__ == "__main__":
    # Generate random training data for the neural network
    num_samples = 1000
    ...
    for iteration in range(100):
        ...
        # Print progress
        print(f"Iteration {iteration + 1}, Max Probability: {result[-1]}")
