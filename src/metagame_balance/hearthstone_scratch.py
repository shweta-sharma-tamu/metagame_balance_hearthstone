...
from metagame_balance.BalanceMetaHS import plot_rewards
from metagame_balance.FCNN import FCNN
from metagame_balance.agent_hearthstone.Seq_Softmax_Competitor import SeqSoftmaxCompetitor
...
from metagame_balance.hearthstone.balance.ERG_Meta import ERGMetaData
from metagame_balance.hearthstone.balance.Policy_Entropy_Meta import PolicyEntropyMetaData
from metagame_balance.hearthstone.competition import CompetitorManager
from metagame_balance.hearthstone.datatypes.Constants import DEFAULT_TEAM_SIZE, get_state_size
...
class HSState(State["HSEnvironment"]):
...
class HSStateDelta(StateDelta["HSEnvironment"]):
    def __init__(self, delta_roster: DeltaRoster):
        self.delta_roster = delta_roster

    @classmethod
    def decode(cls, encoded_next_state: np.ndarray, current_state: HSState) -> "HSStateDelta":
...
class HSEvaluationResult(EvaluationResult["HSEnvironment"]):
...
def _print_roster(roster: HsRoster):
    for p in roster:
        logging.info(p, p.hs_id)
        for move in p.move_roster:
            logging.info(move.name, move.power, move.acc, move.max_pp)


class HSEnvironment(GameEnvironment):
    @property
    ...
    def __init__(self,
        ...
        n_hs_epochs = n_battles_per_league

        # number of championships to run
        self.n_hs_epochs = n_hs_epochs
        ...
        if not roster_path:
            base_roster = RandomHsRosterGenerator(None, n_moves_hs=4, roster_size=BASE_ROSTER_SIZE).gen_roster()
        else:
            import pickle
        ...
        self.metadata.set_moves_and_hs(base_roster)
        ...
        self.hs = ChampionshipEcosystem(base_roster, self.metadata, False, False, strategy=Strategy.RANDOM_PAIRING,
                                         team_size=team_size)

        for a in surrogate:
            self.hs.register(a)

    def get_state(self) -> HSState:
        return HSState(self.metadata)

    def reset(self) -> HSState:
    ...
    def apply(self, state_delta: HSStateDelta) -> \
            HSState:
        self.metadata.update_metadata(delta=state_delta.delta_roster)
        return self.get_state()

    def evaluate(self) -> HSEvaluationResult:
        # train evaluator agents to convergence
        self.hs.run(self.n_hs_epochs, n_league_epochs=self.n_league_epochs) #required to calculate entropy
        agent = next(filter(lambda a: a.competitor.name == "agent", self.hs.league.competitors))
        ...
        return HSEvaluationResult(reward)

    def sample_payoff(self):
        from itertools import combinations
        logging.info("Simulating sample payoff")
        num_hs = len(self.metadata._hs)
        possible_teams = list(combinations(range(num_hs), 2))
        ...
        for p1 in range(len(possible_teams)):
            for p2 in range(len(possible_teams)):
                if p1 == p2:
                    ...
                    win_prob = self.hs.simulate_n_battles(1, t1, t2)
