...
from metagame_balance.hearthstone.balance import DeltaRoster, DeltaHs
...
from metagame_balance.hearthstone.datatypes.Constants import STATS_OPT_1_PER_MOVE, MAX_MOVE_POWER, MAX_MOVE_ACC, \
    MAX_MOVE_MAX_PP, MAX_PKM_HP


class MetaRosterStateParser:

    def __init__(self, num_hs, consider_hp=False, move_roster=STANDARD_MOVE_ROSTER):
        """
        state vector [move_id_1_feat_1, move_id_1_feat_2, .. move_id_2_feat1, .... hs_1_feat_1, hs_1_feat_2, ..]
        """
    ...
    def length_state_vector(self):
        ...
        if self.consider_hp:
            return moves_length + self.num_hs
        else:
            return moves_length
    ...
    def metadata_to_state(self, meta_data: MetaData) -> np.ndarray:
        """
        Takes meta data, uses _hs and _moves to convert into a state vector
        """
        state_vec = copy.deepcopy(self.base_state)
        for hs in meta_data._hs:  ## implement getter setter to avoid this

            delta_move_dict = {}
            for i, move in enumerate(hs.move_roster):
            ...
            if self.consider_hp:
                hs_idx = len(self.move_roster) * STATS_OPT_1_PER_MOVE + hs.hs_id
                state_vec[hs_idx] = max(1, np.round(hs.max_hp))  # Make sure an HP of atleast 1
    ...
    def state_to_delta_roster(self, state_vec: np.ndarray, meta_data: MetaData) -> DeltaRoster:
        ...
        for hs in meta_data._hs:  ##implement getter setter to avoid this
            ...
            if self.consider_hp:
                hs_idx = len(self.move_roster) * STATS_OPT_1_PER_MOVE + hs.hs_id  ### get idx of pokemon HP from here
                hp = max(1, np.round(state_vec[hs_idx]))
            else:
                hp = hs.max_hp
            delta_dict[hs.hs_id] = DeltaHs(hp, hs.type, delta_move_dict)  # update max hhp here!
