...
from metagame_balance.hearthstone.datatypes.Constants import TYPE_CHART_MULTIPLIER, DEFAULT_PKM_N_MOVES, DEFAULT_PARTY_SIZE
from metagame_balance.hearthstone.datatypes.Objects import HsMove, GameStateView
from metagame_balance.hearthstone.datatypes.Types import HsStat, HsType, WeatherCondition, HsStatus


def estimate_damage(move_type: HsType, hs_type: HsType, move_power: float, opp_hs_type: HsType,
                    attack_stage: int, defense_stage: int, weather: WeatherCondition) -> float:
    stab = 1.5 if move_type == hs_type else 1.
    if (move_type == HsType.WATER and weather == WeatherCondition.RAIN) or (
            move_type == HsType.FIRE and weather == WeatherCondition.SUNNY):
        weather = 1.5
    elif (move_type == HsType.WATER and weather == WeatherCondition.SUNNY) or (
            move_type == HsType.FIRE and weather == WeatherCondition.RAIN):
        weather = .5
    else:
        weather = 1.
    stage_level = attack_stage - defense_stage
    stage = (stage_level + 2.) / 2 if stage_level >= 0. else 2. / (np.abs(stage_level) + 2.)
    damage = TYPE_CHART_MULTIPLIER[move_type][opp_hs_type] * stab * weather * stage * move_power
    return damage
...
class SimpleBattlePolicy(BattlePolicy):
    ...
    def get_action(self, g: GameStateView) -> int:
        ...
        my_active = my_team.active_hs_view
        my_active_type = my_active.type
        my_party = [my_team.get_party_hs_view(i) for i in range(my_team.party_size)]
        my_active_moves = [my_active.get_move_view(i) for i in range(DEFAULT_PKM_N_MOVES)]
        my_attack_stage = my_team.get_stage(HsStat.ATTACK)

        # get opp team
        opp_team = g.get_team_view(1)
        opp_active = opp_team.active_hs_view
        opp_active_type = opp_active.type
        opp_active_hp = opp_active.hp
        opp_defense_stage = opp_team.get_stage(HsStat.DEFENSE)
        ...
        if opp_active_hp > damage[move_id]:
            ...
            for i, hs in enumerate(my_party):
                effectiveness_party = TYPE_CHART_MULTIPLIER[hs.type][opp_active_type]
                if effectiveness_party > effectiveness_to_stay and hs.hp != 0.0:
                    effectiveness_to_stay = effectiveness_party
                    best_hs = i
        if best_hs > 0:
            if np.random.uniform() > 0.5:
                move_id = DEFAULT_PKM_N_MOVES + best_hs
...
class GUIBattlePolicy(BattlePolicy):
    ...
    def get_action(self, g: GameStateView) -> int:
        ...
        opp_text = 'Opp: ' + opp_active_type.name + ' ' + str(opp_active_hp) + ' HP' + (
            '' if opp_status == HsStatus.NONE else opp_status.name)
        opp_attack_stage = opp_team.get_stage(HsStat.ATTACK)
        if opp_attack_stage != 0:
            opp_text += ' ATK ' + str(opp_attack_stage)
        opp_defense_stage = opp_team.get_stage(HsStat.DEFENSE)
        if opp_defense_stage != 0:
            opp_text += ' DEF ' + str(opp_defense_stage)
        opp_speed_stage = opp_team.get_stage(HsStat.SPEED)
        ...
        active_text = 'You: ' + my_active_type.name + ' ' + str(my_active_hp) + ' HP' + (
            '' if my_status == HsStatus.NONE else my_status.name)
        active_attack_stage = my_team.get_stage(HsStat.ATTACK)
        if active_attack_stage != 0:
            active_text += ' ATK ' + str(active_attack_stage)
        active_defense_stage = my_team.get_stage(HsStat.DEFENSE)
        if active_defense_stage != 0:
            active_text += ' DEF ' + str(active_defense_stage)
        active_speed_stage = my_team.get_stage(HsStat.SPEED)
        if active_speed_stage != 0:
            active_text += ' SPD ' + str(active_speed_stage)
        self.active.Update(active_text)

        # party
        my_party = [my_team.get_party_hs_view(0), my_team.get_party_hs_view(1)]
        for i, hs in enumerate(my_party):
            party_type = hs.type
            party_hp = hs.hp
            party_status = hs.status
            party_text = party_type.name + ' ' + str(party_hp) + ' HP' + (
                '' if party_status == HsStatus.NONE else party_status.name) + ' '
        ...
        for i, move in enumerate(my_active_moves):
            ...
            move_type = move.type
            self.moves[i].Update(str(HsMove(power=move_power, move_type=move_type)))
...
class BetterRandomBattlePolicy(BattlePolicy):
    ...
    def get_action(self, g: GameStateView) -> int:
        ...
        my_team = g.get_team_view(0)
        pokemon_view = my_team.active_hs_view
