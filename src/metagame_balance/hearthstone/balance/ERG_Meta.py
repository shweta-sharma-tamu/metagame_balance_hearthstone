...
class ERGMetaData(PolicyEntropyMetaData):
    ...
    def get_balanced_payoff(self):
        ...
        possible_teams = list(combinations(range(len(self._hs)), 2))
    ...
    def evaluate(self, winrates) -> float:
        ...
        """
        for t1p1 in range(len(self._hs)):
            for t1p2 in range(len(self._hs)):
                for t2p1 in range(len(self._hs)):
                    for t2p2 in range(len(self._hs)):
