...
from metagame_balance.hearthstone.behaviour.TeamPredictors import NullTeamPredictor
...
from metagame_balance.hearthstone.datatypes.Constants import DEFAULT_MATCH_N_BATTLES, DEFAULT_TEAM_SIZE
from metagame_balance.hearthstone.datatypes.Objects import HsFullTeam, get_full_team_view, HsTeamPrediction, HsFullTeamView, HsTeam
from metagame_balance.hearthstone.engine.HsBattleEnv import HsBattleEnv
from metagame_balance.hearthstone.util.generator.HsTeamGenerators import HsTeamGenerator


def team_selection(c: Competitor, full_team: HsFullTeam, my_team_view: HsFullTeamView,
                   opp_team_view: HsFullTeamView, full_team_size) -> HsTeam:
    try:
...
class BattleMatch:
    ...
    def __team_prediction(self, c: Competitor, opp_team_view: HsFullTeamView) -> HsTeamPrediction:
    ...
    def __run_battle(self, a0: BattlePolicy, a1: BattlePolicy, team0: HsTeam, team1: HsTeam,
                     pred0: Optional[HsTeamPrediction], pred1: Optional[HsTeamPrediction]) -> int:
        env = HsBattleEnv((team0, team1), self.debug, [pred1, pred0])
        s = env.reset()
...
class RandomTeamsBattleMatch(BattleMatch):
    ...
    def __init__(self, gen: HsTeamGenerator, competitor0: CompetitorManager, competitor1: CompetitorManager,
                 n_battles: int = DEFAULT_MATCH_N_BATTLES, debug: bool = False, render: bool = False,
