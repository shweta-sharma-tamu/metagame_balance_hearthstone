...
from metagame_balance.hearthstone.datatypes.Constants import TYPE_CHART_MULTIPLIER
...
N_TYPES = len(list(map(int, HsType)))


def get_effective(t: HsType) -> HsType:
    """
    ...
    if not s:
        return HsType(random.randrange(N_TYPES))
    return HsType(random.choice(s))


def get_super_effective(t: HsType) -> HsType:
    """
...
def get_non_very_effective(t: HsType) -> HsType:
    """
...
N_STATUS = len(list(map(int, HsStatus)))


# Pokemon battle stats
class HsStat(IntEnum):
    ATTACK = 0
    DEFENSE = 1
    SPEED = 2


N_STATS = len(list(map(int, HsStat)))
MAX_STAGE = 5
MIN_STAGE = -5
...
# Pokemon battle stats
class HsEntryHazard(IntEnum):
    NONE = 0
    SPIKES = 1


N_ENTRY_HAZARD = len(list(map(int, HsEntryHazard)))
N_HAZARD_STAGES = 3
