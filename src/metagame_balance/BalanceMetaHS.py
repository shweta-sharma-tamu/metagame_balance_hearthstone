...
from metagame_balance.hearthstone.ecosystem.GameBalanceEcosystem import GameBalanceEcosystem
...
def run(args):
    ...
    This runs for `n_epochs' stage 1 optimization epochs 'n_hs_epochs' stage 2 optimization epochs
    ...
    n_hs_epochs = args.n_hs_epochs
    n_league_epochs = args.n_league_epochs
    population_size = args.population_size

    if args.roster_path == '':
        base_roster = RandomHsRosterGenerator(None, n_moves_hs=4, roster_size=NUM_PKM).gen_roster()
    else:
        import pickle
    ...
    surrogate_agent = [CompetitorManager(SeqSoftmaxCompetitor(agent_name, utility_fn_manager, None)) for agent_name in
                       agent_names]
    constraints = HSDesignConstraints(base_roster)
    ...
    meta_data.set_moves_and_hs(base_roster)
    ...
    gbe.run(n_epochs=n_epochs, n_hs_epochs=n_hs_epochs, n_league_epochs=n_league_epochs)
...
def main():
    ...
    parser.add_argument('--n_hs_epochs', type=int, default=1)
